...
describe("index", () => {
    ...
    beforeEach(async () => {
        ...
        cache = JSON.parse(
            await fs.readFile(path.join("mock", "yamli.json"), "utf-8"),
        );
    });
    ...
    describe("initCache", () => {
        it("should respond with cached value", () => {
            ...
            Yamli.I.SXHRData.start(
                mockRequest,
                `http${TRANSLITERATE_ENDPOINT}word=3bd`,
                null,
            );
        ...
        it("should cache the new word if it is alphanumeric", () => {
            ...
            Yamli.I.SXHRData.start(
                mockRequest,
                `http${TRANSLITERATE_ENDPOINT}word=abusuhayla`,
                null,
            );
            ...
            expect(originalMockRequest._responseCallback).toHaveBeenCalledTimes(
                1,
            );
        });
        ...
        it("should not cache the new word if it is not alphanumeric", () => {
            ...
            Yamli.I.SXHRData.start(
                mockRequest,
                `http${TRANSLITERATE_ENDPOINT}word=$$$`,
                null,
            );

            mockRequest._responseCallback(
                `{"r":"a\\/0|b\\/1","serverBuild":"5515","staleClient":false,"w":"$$$"}`,
            );

            // biome-ignore lint/complexity/useLiteralKeys: the cache key includes characters that cannot use dot notation
